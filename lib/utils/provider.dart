...
class UserProfile with ChangeNotifier {
  ...
  UserProfile(this._nickName, this._avatarUrl);
  ...
  set avatarUrl(String avatarUrl) {
    ...
    SPUtils.saveAvatarUrl(avatarUrl);
    notifyListeners();
  }
}
...
int getCurPlatform() {
  ...
  print('当前平台${isAndroid ? 'true' : 'false'}');
  if (isAndroid) {
    index = 0;
  } else {
    index = 1;
...
class AppInfo with ChangeNotifier {
  ///判断当前App的平台， 0 安卓 ； 1 苹果
  int _platformIndex;

  AppInfo(this._platformIndex);

  int get platformIndex => _platformIndex;

  set platformIndex(int index) {
